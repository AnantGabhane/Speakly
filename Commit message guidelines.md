...
10. BREAKING CHANGE: a commit that has a footer BREAKING CHANGE:, or appends `!` after the type/scope, introduces a breaking API change (correlating with MAJOR in Semantic Versioning). A BREAKING CHANGE can be part of commits of any type.
    * Examples:
      ...
      * Commit message with a breaking change footer and ! to draw attention to breaking change
         ...
         BREAKING CHANGE: This commit includes a breaking change
         ```
11. enhance: A commit of the type enhance introduces an enhancement or improvement to existing functionality (this correlates with MINOR in Semantic Versioning).
    * Examples:
      * Commit message with only enhance type
         ```text
         enhance: improve error messages
         ```
      * Commit message with enhance type, scope and description
         ```text
         enhance(api): improve error messages for validation
         ```
      * Commit message with enhance type and breaking change
         ```text
         enhance!: improve configuration handling
